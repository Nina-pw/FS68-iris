...
async function issueTokensAndRedirect(res: any, user: any, to = "/home") {
  ...
  // ช่วยตัดสินใจจาก URL จริง ไม่ใช่ NODE_ENV เฉย ๆ
  const isHttps =
    (process.env.FRONTEND_URL || "").startsWith("https://") ||
    (process.env.BACKEND_URL || "").startsWith("https://");

  // ถ้าคนละออริจิน (localhost:5173 ↔ localhost:3000) ใช้ LAX จะชัวร์กว่า
  const sameSite: "lax" | "none" = isHttps ? "none" : "lax";
  // หมายเหตุ: ถ้า sameSite = "none" จำเป็นต้อง secure = true และต้องรัน HTTPS เท่านั้น

  res.cookie("refreshToken", refreshToken, {
    httpOnly: true,
    secure: isHttps, // local (http) → false, prod https → true
    sameSite, // http local → "lax", prod https → "none"
    path: "/",
  ...
  res.redirect(
    `${FRONTEND_URL}${to}?token=${accessToken}&avatar=${encodeURIComponent(
      user.avatar || ""
    )}`
  );
}
...
async function findOrCreateByEmail(email: string, displayName?: string) {
  ...
  if (found.length > 0) {
    ...
    if (!found[0].emailVerifiedAt) {
      ...
      const refreshed = await dbClient
        .select()
        .from(usersTable)
        .where(eq(usersTable.userID, found[0].userID));
      return refreshed[0];
