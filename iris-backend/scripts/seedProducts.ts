...
function isAlreadyPublicMinioUrl(url: string) {
  // ถ้า url เป็น public base (เช่น http://localhost:9000) ก็ไม่ต้องอัปโหลดซ้ำ
  return url.startsWith(MINIO_PUBLIC_BASE + "/");
}

async function uploadFromUrlToMinio(url: string | null | undefined) {
  if (!SEED_TO_MINIO || !url) return url ?? null;

  // ⛳️ กัน re-upload จาก MinIO เดิมของเรา
  if (isAlreadyPublicMinioUrl(url)) {
    return url; // ใช้ URL เดิมไปเลย
  }
  ...
  for (const item of SEED) {
    ...
    if (item.variants?.length) {
      for (const v of item.variants) {
        const sourceForVariant = v.imageUrl;
// ถ้ามีรูปเฉพาะของ variant ค่อยอัปโหลด, ถ้าไม่มีให้ reuse primaryImageUrl
        const vImg = sourceForVariant
          ? await uploadFromUrlToMinio(sourceForVariant)
